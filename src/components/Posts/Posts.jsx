...
export default function Posts({ searchTerm, userName, tag }) {
  ...
  const handleFilterChange = (selectedFilter) => {
    ...
    setSelectedButton(
      selectedFilter === NEW ? 1 : selectedFilter === UPVOTED ? 2 : 3
    );
  };
  ...
  useEffect(() => {
    ...
    const sortPosts = (result) => {
      ...
      if (filter === NEW) {
        ...
        sortedPosts = result.sort((a, b) =>
          Object.keys(b).includes(UPVOTED_BY)
            ? Object.keys(b.upvotedBy).length - Object.keys(a.upvotedBy).length
            : -1
        );
      } else if (filter === COMMENTED) {
        sortedPosts = result.sort((a, b) =>
          Object.keys(b).includes(HAS_cOMMENT)
            ? Object.keys(b.hasComment).length -
              Object.keys(a.hasComment).length
            : -1
        );
      }
      return sortedPosts;
    ...
    if (searchTerm) {
      result = result.filter((el) => {
        return (
          el.title
            .split(" ")
            .filter((el) =>
              el.toLowerCase().startsWith(searchTerm.toLowerCase())
            ).length > 0
        );
      });
      ...
      result = result.filter(
        (el) => el.topic.split(" ").join("") === params.id
      );
    }
    ...
    let filteredPosts = user
      ? sortedPosts
      : sortedPosts.slice(
          0,
          sortedPosts.length <= 10 ? sortedPosts.length : 10
        );
    // Adjust view for showing posts in Account view
    setRenderedPosts(
      userName
        ? filteredPosts.filter((el) => el.author === userName)
        : filteredPosts
    );
